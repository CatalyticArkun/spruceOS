...
The observed xmb.7z regression is a multi-factor interaction across layers, not an install-policy defect introduced inside `archiveUnpacker.sh`.
...
4. **Secondary persistence amplifier (device constraints)**
   - Low-memory extraction interruption can leave `.7z.extracting` artifacts that require later retry.
...
- Under arkun-dev, pre_cmd-only normal boot no longer drains startup lanes broadly; lane mismatch or misrouting leaves archives resident longer and repeatedly eligible when an all-mode caller later runs (firstboot/recovery, ThemeGarden exit, or manual all-mode invocation).
- `.7z.extracting` recovery and silent pre_cmd worker changes affect retry mechanics and visibility, but they are not the primary trigger.

Primary trigger vs contributors:
- **Primary regression trigger:** orchestration narrowing from all-mode to pre_cmd on normal boot.
- **Latent defects:** lane/content design debt and staging typo bug.
- **Secondary unresolved amplifier:** low-memory extraction failures that can prolong persistence.
...
Overall assessment:
- As a containment/stabilization move, reverting normal boot toward Development-style all-lane draining is low-risk architecturally and keeps policy in orchestration, not unpacker internals.
...
MiyooMini-family hardware operates with tight RAM margins, and this affects extraction reliability:

- `7zr x` can fail or be interrupted under memory pressure, especially when multiple startup tasks overlap.
- Persistent `.7z.extracting` files may therefore represent interrupted extraction/recovery state.
- Memory pressure can amplify persistence and retry behavior across boots.
- Development’s broader all-lane draining may have masked some of this by retrying and converging queues more often.

Interpretation:
- Memory pressure is a **secondary contributing factor** and an amplifier.
- Memory pressure alone does **not** explain repeated eligibility loops; those loops still require staging/orchestration conditions that keep artifacts in active scan lanes.
- Current evidence does **not** prove memory pressure is the primary root cause of this branch regression.
...
1. **Producer / staging layer (primary ownership)**
   - Owns correct lane assignment at production/staging time.
...
2. **Caller / orchestration layer (primary ownership for runtime behavior)**
   ...
   - Must prevent stranded startup-critical work and provide convergence/failure observability.
...
3. **`archiveUnpacker.sh` (execution engine ownership)**
   ...
   - Can improve diagnostics.
   - Should not embed archive-specific install policy or artifact lifecycle semantics.
...
- lane/staging contracts are still evolving,
- avoiding new policy/orchestration complexity is important.

Prefer **Option B** when:
- minimizing normal-boot work is critical,
- orchestration responsibilities are clearly defined,
- lane ownership and lifecycle contracts are already reliable.

Conclusion:
- **Option A is the safer short-term stabilization path.**

### Extraction Failure Visibility

Add lightweight logging that distinguishes:
- archive discovered / eligible,
- extraction start,
- extraction failure,
- `.7z.extracting` recovery retry,
- final convergence/removal.

Purpose:
- separate wrong-lane persistence,
- orchestration-driven repeated eligibility,
- extraction-failure retry loops on constrained hardware.
...
3. MiyooMini-class low-memory scenarios (where practical):
...
5. Convergence under intermittent failure:
   - interrupt extraction once,
   - leave `.7z.extracting`,
   - boot multiple times,
   - confirm eventual convergence/removal after a successful retry.
6. Distinguish loop classes during validation:
   - repeated eligibility loops,
   - repeated extraction-failure loops.
7. Repeated-cycle tests:
