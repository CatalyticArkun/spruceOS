...
if device_system_handles_sdcard_unmount; then
    ...
    if [ "$s2_arg" = "--reboot" ]; then
        power_trace_emit "REBOOT_BEGIN" "AUTO" "BOOTING" "RUNNING" "systemd_path" "save_poweroff.sh:systemd" "handoff to platform reboot command" "" "reboot" "" "" "" ""
        device_run_reboot_cmd
    else
        power_trace_emit "SHUTDOWN_COMPLETE" "AUTO" "OFF" "OFF" "systemd_path" "save_poweroff.sh:systemd" "platform manages shutdown sequence directly" "" "normal" "" "" "" ""
        run_poweroff_cmd
    fi
