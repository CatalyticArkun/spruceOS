    ...
    case "$line" in
      RESULT\ *)
        ...
        printf '{"type":"%s","run_id":"%s","boot_id":"%s","timestamp_utc":"%s","uptime_seconds":%s,"clock_sane":%s,"id":"%s","verdict":"%s","severity":"%s","confidence":"%s","evidence":"%s","artifact":"%s"}\n' \
          ...
          "$(json_escape "$id")" "$(json_escape "$verdict")" "$(json_escape "$severity")" "$(json_escape "$confidence")" "$(json_escape "$evidence")" "$(json_escape "results/${kind}_results.txt")"
        ;;
    esac
  done < "$file"
}
...
: > "$FINDINGS"
json_result_lines "check" "$RUN_DIR/results/check_results.txt" >> "$FINDINGS"
json_result_lines "check" "$RUN_DIR/results/baseline_check_results.txt" >> "$FINDINGS"
json_result_lines "verifier" "$RUN_DIR/results/verifier_results.txt" >> "$FINDINGS"

fail_count=$(grep -h -c 'verdict=FAIL' "$RUN_DIR/results"/*_results.txt 2>/dev/null | awk '{s+=$1} END{print s+0}')
warn_count=$(grep -h -c 'verdict=WARN' "$RUN_DIR/results"/*_results.txt 2>/dev/null | awk '{s+=$1} END{print s+0}')
info_count=$(grep -h -c 'verdict=INFO' "$RUN_DIR/results"/*_results.txt 2>/dev/null | awk '{s+=$1} END{print s+0}')
ok_count=$(grep -h -Ec 'verdict=(PASS|OK)' "$RUN_DIR/results"/*_results.txt 2>/dev/null | awk '{s+=$1} END{print s+0}')

{
  ...
  echo
  echo "Top findings"
  grep -h '^RESULT ' "$RUN_DIR/results"/*_results.txt 2>/dev/null | head -n 20
  echo
  echo "Recommended verifiers"
  [ "$fail_count" -gt 0 ] || [ "$warn_count" -gt 0 ] && echo "RUN_TEST_V-01.lock" && echo "RUN_TEST_V-02.lock" || echo "none"
  echo
