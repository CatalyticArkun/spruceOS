...
bundle_outputs() {
    ...
    (cd "$RUN_DIR" && tar -czf "$RUN_DIR/bundles/telemetry_bundle.tgz" \
      ...
      curated/signature_counts.txt 2>/dev/null || true)
...
if [ "$PHASE" = "B" ]; then
    run_step "07_phase_b" phase_b_exports
fi

run_step "08_recommend" write_recommendations
run_step "09_telemetry" write_telemetry
run_step "10_summary" summarize_outputs
