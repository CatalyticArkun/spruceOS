...
if [ "$count" -gt 0 ]; then
  echo "RESULT id=P-01 verdict=WARN severity=P1 confidence=medium evidence=pyui_exception_markers_${count}"
else
  echo "RESULT id=P-01 verdict=PASS severity=P4 confidence=high evidence=no_pyui_exception_markers"
fi
